...
def read_current_station():
    try:
        with open('current_station.txt', 'r') as file:
            station = file.read().strip()
            return station if station else 'R20'
...
def main():
    ...
    # Output the combined results with station metadata
    output_data = {
        "station_id": station,
        "timestamp": int(time.time()),
        "arrivals": combined_results
    }
