...
(sf.items = {
  ...
  (sf.display = {
     ...
     */
    loadRow: (data, row) => {
      // Check if ETA is 0 (train arriving now) and add highlight class
      if (data.scheduled !== undefined && parseInt(data.scheduled) === 0) {
        row.addClass('arriving-now');
      } else {
        row.removeClass('arriving-now');
      }
